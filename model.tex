...
\begin{equation}
  ...
  \end{cases}
\end{equation}
The only new parameters are $P(\ti{v_i}{t+1}|\ti{v_i}{t+1})$ the
...
Poisson distribution with expected value $\lambda$.  Thus the
probability density of $y$ given that it is a false alarm is
\begin{equation*}
  P(y|\text{FA}) = \lambda \NormalE{\mu_0}{\Sigma_0}{y}.
\end{equation*}

\marginpar{Separate state probability from observation probability.}
...
We use a Poisson distribution with a small mean to model
$N_{\text{new}}$; at most times there are no new states.  We draw the
positions and velocities of the new states from the same distribution
for initial states.

\marginpar{Separate state probability from observation probability.}
For decoding, if you attribute the observations $y_1^{N_{\text{new}}}$
to new targets at time $t$, add $\sum_k^{N_{\text{new}}}
\log(P(y_k|\text{new}))$ and $\log \left(
...
edges while false alarms will happen in other areas more frequently,
I expect better performance by using separate mechanisms to the two
kinds of events.
