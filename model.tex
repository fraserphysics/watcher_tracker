...
\emph{filtered} forecast\footnote{$P(S(t)|y_1^{t-1})$ is the entire a
  posteriori distribution.}, ie,
\begin{equation*}
  P(S(t)|y_1^{t-1}).
\end{equation*}
...
\begin{align}
  ...
  \Sigma_{t+1}^{-1} & = O^T\Sigma_O O + \left( \Sigma_D + A \Sigma_t
    A^T \right)^{-1} \\
  \label{eq:new_mu}
  \mu_{t+1} &= A \mu_t + \Sigma_{t+1} O^T \Sigma_O^{-1} (\ti{y}{t+1} - OA \mu_t)
\end{align}
...
\begin{align*}
  ...
  &= s^T q s - 2s^T l + c
\end{align*}
where the constant term $c$ is irrelevant.  The quadratic term is
\begin{align*}
