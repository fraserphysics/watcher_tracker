...
\begin{equation}
  \label{eq:1}
   \log \left( P(\bv'|\bv)\right) = \sum_i \log \left(
     P(v_i'|v_i)\right),
\end{equation}
where the subscript $i$ identifies the target.  \textbf{Caution:} I
may have to keep track of logs of determinants or something because
the covariances at a given time are different for different targets
depending on the number of updates applied.
