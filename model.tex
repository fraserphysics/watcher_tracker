...
\item In this step, I want to adjust some $u$ and $v$ values;
  decreasing the $u$s and increading the $v$s.  I select the
...
\item With the adjusted $u_0$ value I recalulate and find
...
false alarms, which corresponds to relaxing the constraint $\sum_i
x_{i,j} \leq 1$ for one value of $j$.  To illustrate, I examine a
variant of the problem in the previous subsection as follows:
\begin{description}
\item[Maximize:] $\sum_{i,j} w_{i,j} x_{i,j}$
\item[Subject to:]
  \begin{align*}
    \sum_i x_{i,j} &\leq 1 ~\forall i\\
    \sum_j x_{i,j} &\leq 1  ~\underline{\forall j \geq 1} \\
    x_{i,j} \geq 0
  \end{align*}
...
\end{equation*}
\end{description}
I modify the Hungarian algorithm as follows: Ignore node $j=0$ until
$u_i < w_{i,0}$ for some $i$, then
\begin{itemize}
\item Remove $(i,j)$ from $X \forall j: (i,j) \in X$
\item Remove $w_{i,0}$ from $w$
\item Remove $u_i$ from $u$
\item Append $(i,0)$ to $Y$, the collection of links to unconstrained
  nodes.
\end{itemize}
