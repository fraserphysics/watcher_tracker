...
Doing a forward interation on a target that is not visible requires
...
For decoding, at time $t$, add $N_{\text{FA}}*\log(P(y|\text{FA}))$ to
the value of $\nu$ for an assignment if it includes $N_{\text{FA}}$
attributions to false alarms.
