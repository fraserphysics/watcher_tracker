...
expanding $u'(B(s,t),s,t)$ and completing the square\footnote{Letting
  $B$ denote $\left( \Sigma_t^{-1} + A^T \Sigma_D^{-1} A \right)^{-1}
  \left( \Sigma_t^{-1} \mu_t + A^T \Sigma_D^{-1} s \right)$, I write
  \begin{align*}
    \nu(s,t+1) &=  -\frac{1}{2}(B-\mu_{t})^T \Sigma_{t}^{-1}
    (B-\mu_{t}) + R_t  - \frac{1}{2} (s-AB)^T \Sigma_{D}^{-1} (s-AB)
    \\
    &\quad - \frac{1}{2}(\ti{y}{t+1}-O s)^T \Sigma_{O}^{-1}
    (\ti{y}{t+1}-Os) \\
    &= s^T q s - 2s^T l + c
  \end{align*}
  where the constant term $c$ is irrelevant and the quadratic, $q$,
  linear, $l$ terms are
  \begin{align*}
    q &= \\
    l &=
